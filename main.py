...
class TelegramBot:
    ...
    def setup_handlers(self):
        ...
        def start_handler(message):
            ...
            if is_subscribed:
                ...
                if not existing_user or (existing_user and not existing_user[0]):
                    self.send_to_all_admins(f"🎉 Новый подписчик: {user.first_name} {user.last_name} (@{user.username})")
        ...
        def text_message_handler(message):
            ...
            # ДОБАВЬТЕ ЭТУ ПРОВЕРКУ ПЕРВОЙ
            if user_id in self.config.ADMIN_ID:
                user_state = self.user_states.get(user_id, {})

                if user_state.get('state') == 'awaiting_delete':
                    self.process_delete_request(message)
                    return

                if user_state.get('awaiting_broadcast'):
                    self.process_broadcast(message)
                    return

            # Обработка кнопок чата для админа
            if user_id in self.config.ADMIN_ID and user_id in self.user_states and 'current_chat' in self.user_states[
                user_id]:
            ...
            # Остальная логика обработки состояний...
            ...
            if user_state == 'NAME':
                ...
                # Если сообщение не обработано, отправляем стандартный ответ
                self.bot.send_message(message.chat.id, "Я не понимаю эту команду. Используйте кнопки меню.",
    ...
    def send_to_all_admins(self, message_text, reply_markup=None):
        """Отправляет сообщение всем администраторам"""
        for admin_id in self.config.ADMIN_ID:
            try:
                self.bot.send_message(admin_id, message_text, reply_markup=reply_markup)
            except Exception as e:
    ...
    def delete_handler(self, message):
        ...
        self.user_states[message.from_user.id] = {'state': 'awaiting_delete'}
    ...
    def get_query(self, message):
        ...
        if application_id:
            ...
            application = self.db.get_application_by_id(application_id)
            if application:
                self.send_to_all_admins(
                    f"📩 Новая заявка!\n{format_application(application)}", reply_markup=get_admin_order_keyboard(application_id)
                )
        else:
    ...
    def info_buttons_handler(self, message):
        """Обработчик кнопок в информационном меню"""
        if message.text == '📞 Контакты':
            self.show_contacts(message)
        elif message.text == '💼 Услуги':
            self.show_services(message)
    ...
    💬 Связь с администратором: @Estetika_admi
    🕒 Время работы: 9:00 - 21:00
    """
        self.bot.send_message(message.chat.id, contacts_text, reply_markup=get_info_keyboard())

    def show_services(self, message):
        """Показ информации об услугах"""
        services_text = """
    💼 Наши услуги:
    ...
    • Детская мебель
    """
        self.bot.send_message(message.chat.id, services_text, reply_markup=get_info_keyboard())
