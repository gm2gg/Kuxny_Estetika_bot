...
class Config:
    ...
    SUBSCRIPTION_SUCCESS = "Спасибо, дизайнер свяжется в ближайшее время. Скажите, Вам нужен проект и расчёт кухни?"
...
class TelegramBot:
    ...
    def setup_handlers(self):
            ...
            if is_subscribed:
                ...
                if not existing_user or (existing_user and not existing_user[0]):
                    self.bot.send_message(self.config.ADMIN_ID,
                                          f"🎉 Новый подписчик: {user.first_name} {user.last_name} (@{user.username})")
        ...
        def text_message_handler(message):
            ...
            if user_id == self.config.ADMIN_ID and user_id in self.user_states and 'current_chat' in self.user_states[
                user_id]:
    ...
    def text_message_handler(self, message):
        ...
        if user_id == self.config.ADMIN_ID and self.user_states.get(user_id, {}).get('awaiting_broadcast'):
            self.process_broadcast(message)
            return

        # Проверяем, ожидаем ли мы номер заявки для удаления
        if user_id == self.config.ADMIN_ID and self.user_states.get(user_id, {}).get('awaiting_delete'):
